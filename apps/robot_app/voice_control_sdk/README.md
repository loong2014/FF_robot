...
- 底层采用 `KWS + ASR + VAD` 双阶段链路
  - `IDLE`：Keyword Spotting 只识别唤醒词 `D-Dog`
  - `ACTIVE`：唤醒后切换到 streaming ASR，持续识别到 VAD 判定静音再结束
...
- `VoiceWakeMapper.buildKeywordsFileContent()` / `VoiceWakeMapper.matchResultLabel()`：用于构造和匹配 D-Dog 的中英混合唤醒别名
- `VoiceCommandMapper.normalizeTranscript()`：把原始转写归一化为可比对文本
- `VoiceConfig` 支持配置唤醒词、模型语言、灵敏度、前导缓存和静音结束阈值
